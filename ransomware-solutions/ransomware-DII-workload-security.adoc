...
[role="lead"]
ストレージ ワークロード セキュリティ (SWS) は、 ONTAP環境のセキュリティ体制、回復性、アカウンタビリティを大幅に強化するNetApp Data Infrastructure Insightsの機能です。SWSはユーザ中心のアプローチを採用し、環境内のすべての認証済みユーザからのすべてのファイル アクティビティを追跡します。高度な分析を使用して、すべてのユーザの通常のアクセス パターンと季節的なアクセス パターンを確立します。これらのパターンは、ランサムウェア シグネチャを使用せずに疑わしい動作を迅速に特定するために使用されます。
...
image:ransomware-solution-attack-results.png["Data Infrastructure Insightsサイトストレージワークロードセキュリティ攻撃結果"]
